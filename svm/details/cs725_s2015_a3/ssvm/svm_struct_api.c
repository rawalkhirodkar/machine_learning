...
/* Modified for Assignment 3, CS 725 2015, IIT Bombay by Aman Madaan
			      ...
			      KERNEL_PARM *kparm)
{
  ...
  //TODO: What should be the size of the feature vector?
  sm->sizePsi=;
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  ...
  doc=*(x.doc); 
  doc.fvec=psi(x,y,sm,sparm); //the feature vector
  ...
  for(class=1;class<=sparm->num_classes;class++) {
    ybar.class=class;   //set the class to be the current class
    doc.fvec=psi(x,ybar,sm,sparm);  //obtain the feature vector corresponding to the current class
    score_ybar=classify_example(sm->svm_model,&doc);
    free_svector(doc.fvec);
    score=loss(y,ybar,sparm)*(1.0-score_y+score_ybar); //what is the score for this y_i?
    //TODO: Write the code to update the best score and best class
  }
  if(bestclass == -1) 
